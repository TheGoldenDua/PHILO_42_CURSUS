...
/*   Updated: 2024/12/26 05:51:44 by del-ganb         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	launch_threads(t_data *data)
{
	...
	{
		if (pthread_create(&data->philo_threads[i], NULL, &philo_monitor,
				&data->philos[i]))
	...
	if (pthread_create(&data->monitor_all_alive, NULL, &monitor_routine, data))
		return (ERROR_THREAD_CREATION);
	if (meals_option_enabled(data) && pthread_create(&data->monitor_all_full,
			NULL, &full_routine, data) != 0)
		return (ERROR_THREAD_CREATION);
	return (CODE_SUCCESS);
}
...
int	wait_for_threads(t_data *simulation)
{
	...
	if ((simulation->total_meals > 0)
		&& pthread_join(simulation->monitor_all_full, NULL))
		return (ERROR_THREAD_JOINING);
	while (++i < nb_philos)
	{
